...
function contentReplacer(options) {
    ...
    return Object.assign({}, toRemoveObject, { '/org/core-web': '/' + options.org + '/' + options.name, '@org/core-web': '@' + options.org + '/' + options.name, '/core-web/': '/' + options.name + '/', '/core-web': '/' + options.name, CoreWeb: strings_1.classify(options.name), 'Core web': strings_1.capitalize(options.name)
            .split('-')
            .join(' '), coreWeb: strings_1.camelize(options.name), CORE_WEB: strings_1.underscore(options.name).toUpperCase(), 'core-web': options.name, '/org/': '/' + options.org + '/', '@org/': '@' + options.org + '/', Org: strings_1.classify(options.org), 'org-': options.org + '-', org: strings_1.camelize(options.org), ORG_: strings_1.underscore(options.org).toUpperCase() + '_' });
}
