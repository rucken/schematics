...
function contentReplacer(options) {
    ...
    return Object.assign({}, toRemoveObject, { '/demo-web-e2e': '/' + options.name, 'demo-web': options.name, 'demo web': options.name.split('-').join(' '), '-e2e-e2e': '-e2e' });
}
function pathReplacer(options) {
    return {
        '/demo-web-e2e': '/' + options.name
    };
}
...
function templateSources(options) {
    return [
        `../../../files/empty/apps/demo-web-e2e`.replace('{basePath}', options.basePath)
    ];
}
...
function updateNxJson(options) {
    return ast_utils_1.updateJsonInTree(core_1.join(core_1.normalize(options.workspace), 'nx.json'), nxJson => {
        ...
        nxJson.projects[name] = {
            tags: ['angular', 'client', 'application', 'e2e', name]
        };
        return nxJson;
    });
}
