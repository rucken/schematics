...
function contentReplacer(options) {
    ...
    return Object.assign({}, toRemoveObject, { '/demo-web/': '/' + options.name + '/', '/demo-web': '/' + options.name, 'demo web': options.name.split('-').join(' '), 'Demo web': strings_1.capitalize(options.name)
            .split('-')
            .join(' '), DemoWeb: strings_1.classify(options.name), 'demo-web': options.name });
}
